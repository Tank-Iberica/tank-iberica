...
function makeSupabase(opts: {
  ...
  const {
    userRole = 'admin',
    dealers = [
      { id: 'd1', company_name: 'Dealer One', avg_response_time_hours: 12, logo_url: 'https://cdn.example.com/logo.png' },
  ...
  const fromImpl = (table: string) => {
    ...
    chain.not = () => chain
    chain.order = () => chain
    chain.range = () => Promise.resolve({ data: table === 'dealers' ? dealers : [], error: dealersError })
    chain.is = () => chain
    chain.maybeSingle = () => {
...
describe('GET /api/admin/dealers/health-scores', () => {
  ...
  it('assigns top badge when score >= 80', async () => {
    mockSupabase = makeSupabase({
      dealers: [{ id: 'd_top', company_name: 'Top Dealer', avg_response_time_hours: 5, logo_url: 'https://cdn.example.com/logo.png' }],
  ...
  it('assigns none badge when score < 60', async () => {
    mockSupabase = makeSupabase({
      dealers: [{ id: 'd_low', company_name: 'Low Dealer', avg_response_time_hours: null, logo_url: null }],
      vehicles: [],
      images: [],
    })
    const result = await healthScoresHandler({} as any)

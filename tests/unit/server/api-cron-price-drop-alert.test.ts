...
describe('price-drop-alert cron', () => {
  ...
  it('skips favorites without user email', async () => {
    ...
    const vehicles = [{ id: 'v1', brand: 'Volvo', model: 'FH', slug: 'volvo-fh' }]
    const favorites = [{ user_id: 'u1', users: null }]
  ...
  it('throws 500 on price_history fetch error', async () => {
    ...
    errChain.then = (r: (v: unknown) => void) =>
      Promise.resolve({ data: null, error: { message: 'DB err' } }).then(r)
    errChain.catch = (r: (v: unknown) => void) =>
      Promise.resolve({ data: null, error: { message: 'DB err' } }).catch(r)
  ...
  it('throws 500 on vehicles fetch error', async () => {
    ...
    vehicleErrChain.then = (r: (v: unknown) => void) =>
      Promise.resolve({ data: null, error: { message: 'vehicles error' } }).then(r)
    vehicleErrChain.catch = (r: (v: unknown) => void) =>
      Promise.resolve({ data: null, error: { message: 'vehicles error' } }).catch(r)
  ...
  it('handles favorites fetch error for a specific vehicle', async () => {
    ...
    favsErrChain.then = (r: (v: unknown) => void) =>
      Promise.resolve({ data: null, error: { message: 'favorites error' } }).then(r)
    favsErrChain.catch = (r: (v: unknown) => void) =>
      Promise.resolve({ data: null, error: { message: 'favorites error' } }).catch(r)
  ...
  it('sends to multiple users when different users favorite different vehicles', async () => {
    ...
    const favsByVehicle: Record<string, unknown[]> = {
      ...
      v2: [
        { user_id: 'u2', users: { id: 'u2', email: 'user2@test.com', name: 'User2', lang: 'en' } },
      ],
    }
      ...
      from: (table: string) => {
        ...
        if (table === 'favorites') {
          ...
          return makeChain([
            {
              user_id: 'u1',
              users: { id: 'u1', email: 'user1@test.com', name: 'User1', lang: 'es' },
            },
          ])
        }
        if (table === 'subscriptions') return makeChain([{ user_id: 'u1', plan: 'premium' }])

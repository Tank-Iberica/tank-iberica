...
describe('createAutoInvoice (billing service)', () => {
  ...
  let supabaseRestGet: typeof import('../../../server/services/billing').supabaseRestGet
  ...
  beforeEach(async () => {
    ...
    supabaseRestGet = mod.supabaseRestGet
  })
  ...
  it('creates invoice with dynamic VAT from dealer fiscal data', async () => {
    mockFetchResponses([
      // 1. Idempotency check: no existing invoice
      { json: [] },
      // 2. Find dealer
      { json: [{ id: 'dealer-1' }] },
      // 3. Fiscal data lookup (Portugal = 23%)
      { json: [{ tax_country: 'PT' }] },
      // 4. Insert invoice (return representation)
      { json: [{ id: 'inv-1' }] },
      // 5. Email notification: fetch user data
      { json: [{ email: 'dealer@test.com', raw_user_meta_data: { display_name: 'Dealer' } }] },
      // 6. Email notification: send
      { json: { success: true } },
    ])
    ...
    // Verify the insert call has correct tax
  ...
  it('skips if stripe_invoice_id already exists (idempotency)', async () => {
    mockFetchResponses([
      // Idempotency: existing invoice found
      { json: [{ id: 'existing-inv' }] },
    ])
  ...
  it('does not skip idempotency when stripeInvoiceId is null', async () => {
    ...
    // No idempotency check call (first call is dealer lookup)
  ...
  it('integrates with Quaderno when API key is set', async () => {
    ...
    mockFetchResponses([
      ...
      // Quaderno API response
      { json: { permalink: 'https://quadernoapp.com/inv/pdf-123' }, ok: true },
      // Insert
      { json: [{ id: 'inv-q' }] },
      // Email user
      { json: [{ email: 'a@b.com', raw_user_meta_data: null }] },
      // Email send
      { json: { success: true } },
    ])
  ...
  it('does not send email when CRON_SECRET is not set', async () => {
    ...
    // Only 4 calls (no email user lookup or email send)

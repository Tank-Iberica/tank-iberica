...
describe('CSS audit', () => {
  ...
  it('no duplicate utility class definitions', () => {
    ...
    const exceptions = ['sr-only', 'page-enter-active', 'page-leave-active', 'layout-enter-active', 'layout-leave-active']
    for (const ex of exceptions) duplicates.delete(ex)

    expect([...duplicates], `Duplicate classes in tokens.css: ${[...duplicates].join(', ')}`).toHaveLength(0)
  })

...
describe('Escalabilidad BD (Fase 8)', () => {
  ...
  describe('Table partitioning readiness (#293)', () => {
    it('has partitioning readiness migration (00087)', () => {
      expect(existsSync(resolve(ROOT, 'supabase/migrations/00087_partitioning_readiness.sql'))).toBe(true)
    })
    ...
    it('has schema separation migration (00088)', () => {
      expect(existsSync(resolve(ROOT, 'supabase/migrations/00088_schema_separation.sql'))).toBe(true)
    })
  })
  ...
  describe('Connection pooling verification (N66)', () => {
    ...
    it('Supabase client is initialized centrally (via #supabase/server)', () => {
      ...
      const serverUtils = readdirSync(resolve(ROOT, 'server/utils')).filter(f => f.endsWith('.ts'))
  ...
  describe('Multi-vertical single deployment (#146)', () => {
    ...
    it('vertical isolation migration exists', () => {
      expect(existsSync(resolve(ROOT, 'supabase/migrations/00062_vertical_isolation.sql'))).toBe(true)
    })

    it('vertical column on vehicles', () => {
      expect(existsSync(resolve(ROOT, 'supabase/migrations/00063_vehicles_vertical_column.sql'))).toBe(true)
    })
  })

/**
 * Tests for app/components/admin/noticias/AdminNewsDeleteModal.vue
 */
import { describe, it, expect } from 'vitest'
import { shallowMount } from '@vue/test-utils'
import DeleteModal from '../../../app/components/admin/noticias/AdminNewsDeleteModal.vue'

describe('AdminNewsDeleteModal', () => {
  const factory = (overrides: Record<string, unknown> = {}) =>
    shallowMount(DeleteModal, {
      props: {
        visible: true,
        articleTitle: 'Tendencias del sector 2026',
        confirmText: '',
        ...overrides,
      },
  ...
  it('shows article title', () => {
    expect(factory().find('.delete-title').text()).toBe('Tendencias del sector 2026')
  })
  ...
  it('disables delete when confirmText is not borrar', () => {
  ...
  it('emits update:confirmText on input', async () => {
    const w = factory()
    const input = w.find('.confirm-input')

...
describe('Multi-Vertical infrastructure (Fase 6)', () => {
  ...
  describe('Inter-vertical data isolation RLS (#254)', () => {
    ...
    it('has analytics vertical isolation tests', () => {
      expect(existsSync(resolve(ROOT, 'tests/unit/server/analytics-vertical-isolation.test.ts'))).toBe(true)
    })
  })
  ...
  describe('Supabase Realtime connection management (N70)', () => {
    const conversation = readFileSync(
      resolve(ROOT, 'app/composables/useConversation.ts'),
      'utf-8',
    )

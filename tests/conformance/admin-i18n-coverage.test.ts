...
describe('Admin pages i18n coverage', () => {
  ...
  const commonKeys = ['common.records', 'common.total', 'common.exportCsv', 'common.loadingItems']
  ...
  describe('No hardcoded title/subtitle text in fixed pages', () => {
    ...
    it('chats.vue unread badge uses $t()', () => {
      const content = readFileSync(resolve(__dirname, '../../app/pages/admin/chats.vue'), 'utf-8')
      expect(content).toContain("$t('admin.chats.unread'")
    })

    it('agenda.vue new contact button uses $t()', () => {
      const content = readFileSync(resolve(__dirname, '../../app/pages/admin/agenda.vue'), 'utf-8')

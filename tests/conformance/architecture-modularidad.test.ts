...
describe('Arquitectura & Modularidad (Fase 7)', () => {
  ...
  describe('Pre-computed aggregates cron (N77)', () => {
    const src = readFileSync(resolve(ROOT, 'server/api/cron/compute-aggregates.post.ts'), 'utf-8')
  ...
  describe('SSE alternative to WebSockets (N78)', () => {
    const src = readFileSync(resolve(ROOT, 'server/api/notifications/stream.get.ts'), 'utf-8')
  ...
  describe('Request coalescing (N83)', () => {
    ...
    it('different keys execute independently', async () => {
      ...
      await Promise.all([coalesce('keyA', 5000, fetcher), coalesce('keyB', 5000, fetcher)])
  ...
  describe('Archival strategy datos >1 año (#298)', () => {
    const retention = readFileSync(resolve(ROOT, 'server/api/cron/data-retention.post.ts'), 'utf-8')

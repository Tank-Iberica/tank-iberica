...
for (const f of files) {
  const rel = f.split('Tracciona/')[1] || f
...
console.log('Directory'.padEnd(38) + 'Cov%'.padStart(6) + '  Missing'.padStart(10) + '  Total'.padStart(8))
console.log('-'.repeat(64))
for (const r of sorted) {
  console.log(r.dir.padEnd(38) + (r.pct+'%').padStart(6) + String(r.missing).padStart(10) + String(r.total).padStart(8))
}
console.log('-'.repeat(64))
const totalMissing = sorted.reduce((a, r) => a + r.missing, 0)
const totalStmts = sorted.reduce((a, r) => a + r.total, 0)
console.log('TOTAL'.padEnd(38) + (Math.round((totalStmts-totalMissing)/totalStmts*100)+'%').padStart(6) + String(totalMissing).padStart(10) + String(totalStmts).padStart(8))

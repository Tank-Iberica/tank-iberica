...
export function useAdminHistorico() {
  ...
  async function fetchEntries(filters: HistoricoFilters = {}) {
    ...
    try {
      let query = supabase
        .from('historico')
        .select('*, subcategories(name_es)', { count: 'exact' })
    ...
    try {
      ...
      if (entry.vehicle_data) {
        ...
        if (subcategory_id && !vehicleData.category_id) {
          vehicleData.category_id = subcategory_id
        }
      } else {
        vehicleData = {
          brand: entry.brand,
      ...
      const { error: deleteErr } = await supabase.from('historico').delete().eq('id', id)
  ...
  async function deleteEntry(id: string): Promise<boolean> {
    ...
    try {
      const { error: err } = await supabase.from('historico').delete().eq('id', id)
  ...
  const summary = computed<HistoricoSummary>(() => {
    ...
    return {
      ...
      totalBeneficio,
      avgBeneficioPercent:
        countWithBenefit > 0 ? Math.round(totalBeneficioPercent / countWithBenefit) : 0,
      byCategory,
      byType,
    }
  })

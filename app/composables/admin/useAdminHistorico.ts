...
export function useAdminHistorico() {
  ...
  async function restoreVehicle(id: string): Promise<boolean> {
    ...
    try {
      ...
      if (entry.vehicle_data) {
        // Clean snapshot: remove joined relations, old id/slug, and stale column names
        const { id: _id, slug: _slug, vehicle_images: _imgs, types: _types, subcategories: _subs, subcategory_id, ...cleanData } = entry.vehicle_data as Record<string, unknown>
        vehicleData = cleanData
        // Fix legacy column name: subcategory_id → type_id
        if (subcategory_id && !vehicleData.type_id) {
          vehicleData.type_id = subcategory_id
        }
      }
      else {
        vehicleData = {
      ...
      // Create new vehicle from the data (new id/slug auto-generated by DB)
      const { error: insertErr } = await supabase
        .from('vehicles')
        .insert({
          ...vehicleData,
          status: 'draft',
          slug: `${entry.brand}-${entry.model}-restored-${Date.now()}`.toLowerCase().replace(/\s+/g, '-'),

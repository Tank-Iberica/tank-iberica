...
export function useAdminSocialCalendar() {
  ...
  const visibleDays = computed<Date[]>(() => {
    ...
    const totalCells = daysInMonth + ((gridStart.getDate() !== start.getDate()) ? (start.getDay() === 0 ? 6 : start.getDay() - 1) : 0)
  ...
  async function fetchCalendar() {
    ...
    error.value = null
    try {
      const resp = await $fetch<{ ok: boolean; posts: CalendarPost[]; from: string; to: string }>('/api/social/calendar', {

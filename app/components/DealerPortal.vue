  ...
  <div class="dealer-portal">
    ...
    <section v-if="hasSocialLinks" class="dealer-section">
      ...
      <div class="social-links">
        <a
          v-for="(url, platform) in visibleSocialLinks"
          :key="platform"
          :href="url"
          target="_blank"
        ...
        <!-- Overflow toggle -->
        <button
          v-if="hasOverflowLinks && !socialOverflowExpanded"
          class="social-link social-overflow-btn"
          :title="t('common.showMore')"
          @click="socialOverflowExpanded = true"
        >
...
const socialOverflowExpanded = ref(false)
const visibleSocialLinks = computed(() => {
  const entries = Object.entries(activeSocialLinks.value)
  if (entries.length <= 4 || socialOverflowExpanded.value) return activeSocialLinks.value
  return Object.fromEntries(entries.slice(0, 4))
})
const hasOverflowLinks = computed(() => Object.keys(activeSocialLinks.value).length > 4)

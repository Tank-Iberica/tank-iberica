...
async function load() {
  ...
  try {
    // Status distribution
    const { data: vehicles, error: vErr } = await supabase.from('vehicles').select('status')
    ...
    if (!eErr && expired) {
      ...
      for (const v of expired as Array<{ dealer_id: string; dealers: { company_name: string } }>) {
        const existing = dealerMap.get(v.dealer_id) ?? {
          company_name: v.dealers.company_name,
    ...
    <template v-else>
      ...
      <section class="report-section">
        ...
        <div class="status-grid">
          <div v-for="row in statusCounts" :key="row.status" class="status-card">
            <div class="status-info">
              <span
                class="status-dot"
                :style="{ background: STATUS_META[row.status]?.color ?? '#999' }"
              />
            ...
            <div class="status-bar-wrap">
              <div
                ...
                :style="{
                  width: `${pct(row.count)}%`,
                  background: STATUS_META[row.status]?.color ?? '#999',
                }"
              />
            </div>
        ...
        <table class="report-table">
          ...
          <tbody>
            <tr v-for="d in dealersWithExpired" :key="d.dealer_id">
              ...
              <td class="text-right">
                <NuxtLink
                  :to="`/admin/vehiculos?dealer=${d.dealer_id}&status=expired`"
                  class="link-action"
                >

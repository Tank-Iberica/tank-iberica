...
const emit = defineEmits<{
  ...
  (e: 'update-confirm-text', text: string): void
}>()

function onConfirmInput(event: Event) {
  const target = event.target as HTMLInputElement
  emit('update-confirm-text', target.value)
}
</script>
    ...
    <div v-if="show" class="modal-overlay" role="dialog" aria-modal="true" @click.self="emit('close')">
      <div class="modal-content modal-small">
        <div class="modal-header">
          <h3>{{ $t('admin.anunciantes.confirmDelete') }}</h3>
          <button class="modal-close" :aria-label="$t('common.close')" @click="emit('close')">×</button>
        </div>
        <div class="modal-body">
          <p v-html="$t('admin.anunciantes.deleteConfirmMsg', { name: `<strong>${advertisementName}</strong>` })" />
          <div class="form-group delete-confirm-group">
            <label for="delete-confirm" v-html="$t('admin.anunciantes.typeDeleteConfirm', { word: '<strong>Borrar</strong>' })" />
            <input
              id="delete-confirm"
              type="text"
              :value="confirmText"
            ...
            <p v-if="confirmText && !canDelete" class="text-error">
              {{ $t('admin.anunciantes.typeDeleteExact', { word: 'Borrar' }) }}
            </p>
          </div>
        </div>

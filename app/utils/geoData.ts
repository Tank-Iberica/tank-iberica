...
export const REGION_ADJACENCY: Record<string, string[]> = {
  'Andalucía': ['Extremadura', 'Castilla-La Mancha', 'Región de Murcia', 'Comunidad Valenciana', 'Ceuta', 'Melilla'],
  ...
  'Castilla y León': ['Galicia', 'Asturias', 'Cantabria', 'País Vasco', 'La Rioja', 'Aragón', 'Castilla-La Mancha', 'Comunidad de Madrid', 'Extremadura'],
  'Castilla-La Mancha': ['Comunidad de Madrid', 'Castilla y León', 'Aragón', 'Comunidad Valenciana', 'Región de Murcia', 'Andalucía', 'Extremadura'],
  'Cataluña': ['Aragón', 'Comunidad Valenciana', 'Islas Baleares'],
  'Comunidad Valenciana': ['Cataluña', 'Aragón', 'Castilla-La Mancha', 'Región de Murcia', 'Andalucía', 'Islas Baleares'],
...
export const EUROPEAN_COUNTRIES: string[] = [
  ...EU_MEMBERS,
  'GB', 'NO', 'CH', 'IS', 'LI', 'AD', 'MC', 'SM', 'ME',
  'RS', 'BA', 'MK', 'AL', 'MD', 'UA', 'BY', 'RU', 'TR',
]

// --- Country names by locale ---
export const COUNTRY_NAMES: Record<string, Record<string, string>> = {
  es: {
    ES: 'España', PT: 'Portugal', FR: 'Francia', GR: 'Grecia',
    DE: 'Alemania', IT: 'Italia', GB: 'Reino Unido', NL: 'Países Bajos',
    BE: 'Bélgica', AT: 'Austria', CH: 'Suiza', PL: 'Polonia',
    CZ: 'Chequia', SK: 'Eslovaquia', HU: 'Hungría', RO: 'Rumanía',
    BG: 'Bulgaria', HR: 'Croacia', SI: 'Eslovenia', SE: 'Suecia',
    DK: 'Dinamarca', FI: 'Finlandia', NO: 'Noruega', IE: 'Irlanda',
    LU: 'Luxemburgo', EE: 'Estonia', LV: 'Letonia', LT: 'Lituania',
    MT: 'Malta', CY: 'Chipre', IS: 'Islandia', AD: 'Andorra',
    MC: 'Mónaco', SM: 'San Marino', LI: 'Liechtenstein',
    ME: 'Montenegro', RS: 'Serbia', BA: 'Bosnia y Herzegovina',
    MK: 'Macedonia del Norte', AL: 'Albania', MD: 'Moldavia',
    UA: 'Ucrania', BY: 'Bielorrusia', RU: 'Rusia', TR: 'Turquía',
  },
  en: {
    ES: 'Spain', PT: 'Portugal', FR: 'France', GR: 'Greece',
    DE: 'Germany', IT: 'Italy', GB: 'United Kingdom', NL: 'Netherlands',
    BE: 'Belgium', AT: 'Austria', CH: 'Switzerland', PL: 'Poland',
    CZ: 'Czechia', SK: 'Slovakia', HU: 'Hungary', RO: 'Romania',
    BG: 'Bulgaria', HR: 'Croatia', SI: 'Slovenia', SE: 'Sweden',
    DK: 'Denmark', FI: 'Finland', NO: 'Norway', IE: 'Ireland',
    LU: 'Luxembourg', EE: 'Estonia', LV: 'Latvia', LT: 'Lithuania',
    MT: 'Malta', CY: 'Cyprus', IS: 'Iceland', AD: 'Andorra',
    MC: 'Monaco', SM: 'San Marino', LI: 'Liechtenstein',
 ...
 */
export function getSortedEuropeanCountries(locale = 'es'): { priority: CountryEntry[], rest: CountryEntry[] } {
  const names = COUNTRY_NAMES[locale] || COUNTRY_NAMES.es!
  const toEntry = (c: string): CountryEntry => ({ code: c, name: names[c] || c, flag: countryFlag(c) })

  const priority = PRIORITY_COUNTRIES
    .filter(c => EUROPEAN_COUNTRIES.includes(c))
    .map(toEntry)

  const rest = EUROPEAN_COUNTRIES
...
export function getAvailableLevels(country: string | null): LocationLevel[] {
  ...
  if (country === 'ES') {
    return ['provincia', 'comunidad', 'limitrofes', 'nacional', 'suroeste_europeo', 'union_europea', 'europa', 'mundo']
  }

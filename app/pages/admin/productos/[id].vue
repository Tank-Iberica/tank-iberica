...
async function loadVehicle() {
  ...
  formData.value = {
    ...
    filters_json: (data.filters_json as Record<string, unknown>) || {},
    status: data.status,
...
async function handleImageUpload(e: Event) {
  ...
  alert('La subida de imágenes requiere configuración de Cloudinary.')
...
const finalProfit = computed(() => {
  return (sellData.value.sale_price || 0) - totalCost.value - (sellData.value.sale_price * sellData.value.commission / 100)
})
  ...
  <div class="pf">
    ...
    <template v-else-if="vehicle">
      ...
      <header class="pf-header">
        ...
        <div class="pf-right">
          <button v-if="formData.status !== 'sold'" class="btn btn-sell" @click="showSellModal = true">€ Vender</button>
      ...
      <div class="pf-body">
        ...
        <div class="section">
          ...
          <div v-if="images.length" class="img-grid">
            <div v-for="(img, idx) in images" :key="img.id" class="img-item" :class="{ cover: idx === 0 }">
              <img :src="img.url" :alt="`Imagen ${idx + 1}`">
              <div class="img-overlay">
                <div class="img-actions">
                  <button v-if="idx !== 0" title="Portada" @click="setAsPortada(idx)">⭐</button>
                  <button v-if="idx > 0" title="Mover arriba" @click="moveImage(idx, 'up')">↑</button>
                  <button v-if="idx < images.length - 1" title="Mover abajo" @click="moveImage(idx, 'down')">↓</button>
      ...
      <div v-if="showDeleteModal" class="modal-bg" @click.self="showDeleteModal = false">
        <div class="modal">
          ...
          <div class="modal-body">
            <p>¿Eliminar <strong>{{ vehicle?.brand }} {{ vehicle?.model }}</strong>?</p>
          ...
          <div class="modal-foot">
            ...
            <button class="btn btn-danger" :disabled="!canDelete" @click="executeDelete">Eliminar</button>
          </div>
        </div>
      </div>
    </Teleport>
    ...
    <Teleport to="body">
      <div v-if="showSellModal" class="modal-bg" @click.self="showSellModal = false">
        <div class="modal modal-lg">
          ...
          <div class="modal-body">
            <p>Vender <strong>{{ vehicle?.brand }} {{ vehicle?.model }}</strong></p>
            <div class="row-2">
              <div class="field">
            ...
            <div class="profit-box">
              <div class="profit-row"><span>Precio venta</span><span>{{ fmt(sellData.sale_price) }}</span></div>
              <div class="profit-row"><span>− Coste total</span><span>{{ fmt(totalCost) }}</span></div>
              <div class="profit-row"><span>− Comisión ({{ sellData.commission }}%)</span><span>{{ fmt(sellData.sale_price * sellData.commission / 100) }}</span></div>
...
.pf-left .star { color: #f59e0b; font-size: 1.2rem; }
...
.btn-sell { background: #16a34a; color: #fff; border: none; }
...
.upload-zone:hover { border-color: #23424A; background: #f3f4f6; }
...
.modal-body .warn { color: #f59e0b; font-size: 0.85rem; margin-top: 8px; }
.modal-body .field { margin-top: 12px; }
.modal-body .row-2 { margin-bottom: 12px; }
.modal-foot {
  display: flex;
...
.profit-row.final.pos { color: #16a34a; }
.profit-row.final.neg { color: #dc2626; }

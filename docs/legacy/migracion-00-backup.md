...
2. Copiar TODA la estructura de archivos, incluyendo:
   - /app/ (componentes, páginas, composables, etc.)
   - /supabase/ (migraciones, config)
   - /public/
   - /server/
   - nuxt.config.ts, package.json, tsconfig.json, y todos los archivos raíz
   - .env, .env.example si existen
   - NO copiar node_modules ni .nuxt (se regeneran con npm install)

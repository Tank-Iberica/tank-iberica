...
| **advertisements** | id (uuid PK), user_id (FK), vehicle_type, brand, model, year, price, location, description, photos, contact_*, status (enum), created_at | anunciantes |
| **demands** | id (uuid PK), user_id (FK), vehicle_type, year_min/max, price_min/max, specs (jsonb), contact_*, status (enum), created_at | solicitantes |

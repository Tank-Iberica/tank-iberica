...
| 4   | 10 errores TypeScript restantes                       | M        | Code           | ✅ done               | ✅ Resuelto sesión 23-mar: schema sync completo (00192–00196) añadió 3 tablas + ~10 columnas + 3 RPCs. Types regenerados (9055 líneas). 13 `as any` eliminados. `nuxt typecheck` 0 errores.                                                                                             |
...
| 57  | Km Score badge con logica real (ITVs)            | M        | Code | ✅ done + ✅ Tests | ✅ `dgt-report.post.ts` usa `createReportAdapter()` + `analyzeKmReliability()`. Score calculado de ITV real (no hardcodeado). Fallback a mock si no hay API key.  |
...
| 99  | Rotacion secrets automatizada          | M        | Config  | ✅ done            | ✅ `secrets-rotation-reminder.yml` crea issue GitHub c/6 meses (Feb + Ago). `SECRETS-ROTATION.md` documenta inventario + procedimientos. Bug MONTH corregido. |
...
| 246 | APM Sentry Performance (sampling configurable)                           | S        | Code        | ✅ done            | ✅ Resuelto sesión 23-mar: `sentryTracesSampleRate` configurable via `NUXT_PUBLIC_SENTRY_TRACES_SAMPLE_RATE`. `nuxt.config.ts` + `error-handler.ts` actualizados.                |

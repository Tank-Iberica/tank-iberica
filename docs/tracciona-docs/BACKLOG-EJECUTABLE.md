...
| 221 | select('\*') cleanup ronda 2 (~25 queries residuales)      | M        | Code   | ⚠️ Parcial + ✅ Avance | ⚠️ Aún **~29 instancias** `select('*')` residuales. ✅ **6 corregidas** (Roadmap Autónomo 1.7): valuation.get (3→0), marketReport.ts (2→0), jobQueue (1→0), export.get dealers (1→0, 45 columnas explícitas GDPR). Restantes: useUserProfile (9), useVerticalConfig (1), infra/clusters (10), supabaseQuery (3), otros. |
...
| N21 | Font subsetting Inter (solo caracteres ES+EN)                              | S        | Code | —          | unicode-range o glyphhanger reduce 40-60% peso fuente                            |
...
| N24 | HTTP 304 Not Modified verificado (ETag/Last-Modified)                      | S        | Code | —          | Verificar que Nitro/CF respetan If-None-Match en API responses                   |
| N25 | Critical request chain audit (eliminar recursos que bloquean render)       | S        | Code | —          | Lighthouse chain documentada, eslabones innecesarios eliminados                  |
...
| #   | Item                                                                        | Esfuerzo | Tipo   | Depende de  | Hecho cuando...                                                                               |
| --- | --------------------------------------------------------------------------- | -------- | ------ | ----------- | --------------------------------------------------------------------------------------------- |
| N29 | Rate limiting fallback serverless (sliding window en Supabase)              | M        | Code   | #1          | Tabla rate_limit_entries + RPC check_rate_limit(). Funciona sin Redis ni WAF                   |
...
| N41 | Push notifications end-to-end verificadas                                  | M        | Code | #186       | Service worker registra, permission UI, server envía push, notificación aparece en móvil |
...
| N81 | Email infrastructure at scale (Resend → SES fallback, sender reputation)     | M        | Code | —          | Resend primary + SES fallback. Warming schedule. Bounce/complaint rate monitoring         |
...
| N84 | Connection usage monitoring (alerta Supabase connections >80%)               | S        | Code | #142       | pg_stat_activity count vs max_connections con alerting automático                         |
...
| #   | Item                                                        | Hecho cuando...                                                                                |
| --- | ----------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| 173 | Webhook endpoint registrado (Test mode)                     | ✅ Webhook `we_1TC7M8...` creado via API: checkout.session.completed, subscription.*, invoice.*, charge.refunded. Secret en `.env` |

...
| #   | Item                                                               | Tipo | Depende de | Tests necesarios                                                                                                  | Hecho cuando...                                                                      |
| --- | ------------------------------------------------------------------ | ---- | ---------- | ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| T1  | Tests E2E puntuación dealer (`/dashboard/herramientas/puntuacion`) | Code | #32        | Montar página → verificar score visible → verificar progress bar → verificar criteria list → verificar i18n ES+EN | Tests en `tests/e2e/pages/puntuacion.test.ts`: mount, load, render, translations     |
| T2  | Tests integración verify-document ownership (IDOR)                 | Code | #2         | Usuario NO propietario intenta verificar documento → 403; propietario → 200; admin → 200                          | Tests en `tests/unit/server/api-verify-document.test.ts`: ownership checks, 3 casos  |
| T3  | Tests merchant-feed NO expone service errors                       | Code | #3         | Feed genera sin exposición de nombres internos; errors logged, response genérico                                  | Tests en `tests/unit/server/merchant-feed.test.ts`: error handling, generic messages |
| T4  | Build test: exceljs genera chunk vendor-excel                      | Code | #6         | `npm run build` genera `dist/.nuxt/dist/server/chunks/vendor-excel.*.mjs`                                         | Tests en `tests/build/chunks.test.ts`: build artifact verification                   |

**Total Roadmap Tests:** ~4 sesiones | **Impacto:** Garantiza items #2, #3, #6, #32 seguros para futuro
...
| 2   | `/api/verify-document` sin validacion ownership       | S        | Code           | ✅ done + ⏳ T2         | checkVehicleAccess() verifica dealer_id==session.user.dealer OR role=admin; 403 si no coincide. **Tests pendientes:** T2 en Roadmap Tests                                      |
| 3   | 5 server routes exponen nombres servicio en error     | S        | Code           | ✅ agent-c + ⏳ T3      | merchant-feed.get.ts + embed/[dealer-slug].get.ts: error.message → logger.error + generic text; safeError ya cubre el resto en prod. **Tests pendientes:** T3 en Roadmap Tests |
...
| 6   | exceljs no incluido en manualChunks                   | S        | Code           | ✅ done + ⏳ T4         | `npm run build` muestra chunk `vendor-excel` separado. **Tests pendientes:** T4 en Roadmap Tests                                                                               |
...
| #   | Item                                                                      | Esfuerzo | Tipo | Depende de | Hecho cuando...                                                                                                                                                                                                                                                                                                                                                |
| --- | ------------------------------------------------------------------------- | -------- | ---- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 7   | Actualizar seed credit_packs (5 packs nuevos con bonus)                   | S        | Code | —          | ✅ agent-c + ✅ Verificado — server/api/stripe/checkout-credits.post.ts + seed implementado. Credit packs tabla en BD con 5 packs.                                                                                                                                                                                                                             |
...
| 447 | Fiscal Compliance Engine + Billin adapter + Multi-divisa + IVA multi-país | L        | Code | #8         | ⏳ Parcialmente implementado — Billin adapter básico en `billing.ts`, pero verificar: Multi-divisa (campos `currency`/`exchange_rate` en invoices), IVA multi-país (country_vat_rates migration), OSS reporting, tax nexus cron. **Estado:** adapter Billin existe, falta cobertura completa multi-país/OSS. Dejar como PENDIENTE hasta verificación profunda. |
| 17  | Frontend pagina compra creditos completa                                  | M        | Code | #7         | ✅ agent-c + ✅ Verificado — app/pages/precios.vue + PreciosComparisonTable. Página `/precios` muestra 5 packs con funcionalidad Stripe Checkout, balance actualizado tras success.                                                                                                                                                                            |
...
| #   | Item                                                 | Esfuerzo | Tipo | Depende de | Hecho cuando...                                                                                                                                        |
| --- | ---------------------------------------------------- | -------- | ---- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 62  | Motor recalculo active_landings — cron/edge function | L        | Code | —          | ✅ agent-c + ✅ Verificado — server/api/cron/recalculate-landings.post.ts. Cron semanal ejecuta, combinaciones se activan/desactivan con vehículos.    |
| 63  | Catalogo real en landing pages [...slug].vue         | M        | Code | #62        | ⏳ Parcialmente verificado — Landing pages probablemente existen pero requieren verificación de filtrado VehicleGrid. **Status:** REVISAR completitud. |
| 64  | Schema Vehicle JSON-LD en fichas                     | S        | Code | —          | ✅ agent-c + ✅ Verificado — app/composables/useJsonLd.ts. Schema Vehicle con brand, model, mileageFromOdometer implementado.                          |
...
| 11  | Reserva Prioritaria completa (2 creditos)             | L        | Code | #7, #10    | ✅ agent-c + ✅ Verificado — app/pages/perfil/reservas.vue + server/api/reservations/\*. Pausado 48h, refund automático, Premium inmune implementado. |
...
| 15  | Color/fondo/marco especial anuncios (2 creditos)      | M        | Code | #7         | ⏳ NOT FOUND — No encontrado en búsqueda de código. Probable: estilos especiales anuncios no implementados. **Status:** PENDIENTE.                    |
| 16  | Exportar catalogo gated por creditos                  | S        | Code | #7, #8     | ✅ agent-c + ✅ Verificado — app/pages/dashboard/herramientas/exportar.vue + ExportActionButton. CSV/XLSX descargable.                                |
...
| #   | Item                                                         | Esfuerzo | Tipo | Depende de | Hecho cuando...                                                                                      |
| --- | ------------------------------------------------------------ | -------- | ---- | ---------- | ---------------------------------------------------------------------------------------------------- |
| 35  | Sale price obligatorio en SoldModal                          | S        | Code | —          | ✅ agent-c + ✅ Verificado — Modal SoldModal en dashboard requiere sale_price. Campo guardado en BD. |
...
| 32  | Guia "Mejora tu puntuacion" en dashboard              | S        | Code | #30        | ✅ agent-c + ⏳ T1 — /dashboard/herramientas/puntuacion, progress bar, criteria list, i18n ES+EN. **Tests pendientes:** T1 en Roadmap Tests                                                                                                                 |
...
| #   | Item                               | Esfuerzo | Tipo | Depende de | Hecho cuando...                                                                                                                                                                                              |
| --- | ---------------------------------- | -------- | ---- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 50  | Backend seller_reviews completo    | M        | Code | —          | ⏳ NOT FOUND — No encontrado en búsqueda de código. **Status:** PENDIENTE. Requiere tabla reviews, RLS, API endpoints.                                                                                       |
| 51  | Display reviews en perfil dealer   | M        | Code | #50        | ⏳ BLOQUEADO por #50 — Requiere que #50 esté completo. **Status:** PENDIENTE.                                                                                                                                |
| 52  | Reviews con dimensiones JSONB      | S        | Code | #50        | ⏳ BLOQUEADO por #50 — 4 dimensiones JSONB (communication, accuracy, condition, logistics). **Status:** PENDIENTE.                                                                                           |
| 53  | NPS 0-10 en reviews                | S        | Code | #50        | ⏳ BLOQUEADO por #50 — Campo NPS + promedio visible. **Status:** PENDIENTE.                                                                                                                                  |
| 54  | Badge Top-Rated filtrable          | S        | Code | #50, #30   | ⏳ BLOQUEADO por #50 — Filtro "Solo Top-Rated" + badge. **Status:** PENDIENTE.                                                                                                                               |
| 55  | Scoreboard publico Top 100 dealers | M        | Code | #50, #30   | ✅ agent-c + ✅ Verificado — app/pages/top-dealers.vue. Página `/top-dealers` con ranking de dealers ordenado por rating (nota: depende de #50 para datos reales de reviews, pero página estructura existe). |
...
| 68  | Social auto-post OAuth2 real                          | L        | Code | —          | ✅ agent-c + ✅ Verificado — server/api/social/auto-publish.post.ts + server/api/social/publish.post.ts. OAuth flow LinkedIn/FB/IG/X, publicación automática al aprobar vehículo. |
| 69  | WhatsApp Channel auto-post                            | M        | Code | —          | ✅ agent-c + ✅ Verificado — server/api/whatsapp/broadcast.post.ts. INSERT en vehicles dispara mensaje al canal difusión WhatsApp Business.                                       |
| 70  | Pinterest auto-pin                                    | S        | Code | —          | ✅ agent-c + ✅ Verificado — server/api/pinterest/create-pin.post.ts. Genera pin via Pinterest API al crear vehículo.                                                             |
| 71  | Calendario editorial visual en admin                  | L        | Code | —          | ✅ agent-c + ✅ Verificado — server/api/social/calendar.get.ts + server/api/social/reschedule.patch.ts. Vista semanal/mensual con drag & drop de publicaciones planificadas.      |
...
| 19  | Informe valoracion de mercado               | M        | Code | —          | ⏳ Parcialmente verificado — Probablemente en server/api/market-report/\* pero requiere verificación de datos históricos. **Status:** REVISAR.                       |
| 20  | Comparador vehiculos premium                | M        | Code | —          | ✅ agent-c + ✅ Verificado — app/pages/perfil/comparador.vue + ComparadorHeader.vue + PreciosComparisonTable. Comparación 2-3 vehículos con métricas.                |
| 21  | Historial precio vehiculo                   | M        | Code | —          | ⏳ Parcialmente verificado — price_history probable en BD pero requiere verificación de gráfico/UI. **Status:** REVISAR.                                             |
| 22  | Alertas bajada con umbral                   | S        | Code | —          | ✅ agent-c + ✅ Verificado — server/api/cron/favorite-price-drop.post.ts + server/api/cron/price-drop-alert.post.ts. Umbral de precio implementado.                  |
| 23  | Generacion IA ficha cobrada (1 credito)     | S        | Code | #7         | ⏳ Probable — Pipeline WhatsApp en server/api/whatsapp/\* pero requiere verificación de cobro. **Status:** REVISAR.                                                  |
| 24  | Estadisticas rendimiento anuncio detalladas | M        | Code | #8         | ✅ agent-c + ✅ Verificado — app/pages/dashboard/estadisticas.vue. Dashboard con comparativa mercado + recomendaciones precio.                                       |
| 25  | Recomendacion precio IA                     | M        | Code | —          | ⏳ Parcialmente verificado — Lógica IA probable en composables pero requiere verificación. **Status:** REVISAR.                                                      |
| 26  | Certificado publicacion PDF con QR          | M        | Code | #7         | ⏳ Parcialmente verificado — PDF + QR probable pero requiere verificación de endpoint descarga. **Status:** REVISAR.                                                 |

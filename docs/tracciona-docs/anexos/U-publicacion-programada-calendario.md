# U-publicacion-programada-calendario

Alias de compatibilidad para referencias historicas en docs/legacy/.

Este anexo ya no se mantiene como documento activo independiente.

Consulta primero:
- [README.md](README.md)
- [../../README.md](../../README.md)
- [../../PROYECTO-CONTEXTO.md](../../PROYECTO-CONTEXTO.md)
- [../BACKLOG-EJECUTABLE.md](../BACKLOG-EJECUTABLE.md)
- [../referencia/ARQUITECTURA-ESCALABILIDAD.md](../referencia/ARQUITECTURA-ESCALABILIDAD.md)

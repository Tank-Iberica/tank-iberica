...
- **Cherry-pick entre branches:** Si un commit cae en el branch equivocado, `git cherry-pick <sha>` al branch correcto. Único conflicto habitual: PARALLEL-AGENTS.md (DU = deleted-by-us) → resolver con `git add` del archivo.
- **Git worktree (crítico):** Crear `git worktree add Tracciona-agent-c agent-c/bloque-6b` para aislar de branch switching de otros agentes. Cada worktree tiene su propio HEAD independiente. Comando: `cd /c/TradeBase && git -C Tracciona worktree add Tracciona-agent-c agent-c/bloque-6b`

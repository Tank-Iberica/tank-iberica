...
export async function createAutoInvoice(
  ...
  if (params.stripeInvoiceId) {
    const existing = await supabaseRestGet<{ id: string }>(
      config, 'invoices',
      `stripe_invoice_id=eq.${params.stripeInvoiceId}`, 'id',
    )
    if (existing.length > 0) {
      return { id: existing[0]!.id, dealerId: null, vatRate: 0, taxCountry: '', pdfUrl: null, quadernoConfigured: false }
    }
  }

  const dealerData = await supabaseRestGet<{ id: string }>(
    config, 'dealers', `user_id=eq.${params.userId}`, 'id',
  )
  ...
  if (dealerId) {
    const fiscalData = await supabaseRestGet<{ tax_country: string }>(
      config, 'dealer_fiscal_data', `dealer_id=eq.${dealerId}`, 'tax_country',
    )
  ...
  return { id: invoiceId, dealerId, vatRate, taxCountry, pdfUrl, quadernoConfigured }
}
  ...
  try {
    ...
    await fetch(`${baseUrl}/api/email/send`, {
      ...
      body: JSON.stringify({
        ...
        to: email,
        userId,
        variables: { name, serviceType, amount: `${(amountCents / 100).toFixed(2)} EUR` },
      }),
    })
  } catch {
    // Best-effort

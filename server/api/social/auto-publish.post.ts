...
function buildPostContent(v: VehicleData, platform: SocialPlatform, locale: string): string {
  ...
  const price = v.price ? formatPrice(v.price) : locale === 'es' ? 'Precio a consultar' : 'Price on request'
...
export default defineEventHandler(async (event) => {
  ...
  for (const platform of platforms) {
    ...
    const tokens = (tokenConfig as { value: { access_token?: string; expires_at?: string } } | null)?.value
    ...
    const autoPublishEnabled =
      (autoConfig as { value: boolean } | null)?.value === true
    ...
    // Auto-publish if tokens configured and auto_publish enabled
    if (tokens?.access_token && autoPublishEnabled) {
      ...
      try {
        // Call internal publish endpoint via service role (simplified: call publish logic directly)
        const publishResp = await fetch(`${getSiteUrl()}/api/social/publish`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            // Service-to-service: use internal header

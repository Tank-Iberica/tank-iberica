...
export async function getJob(supabase: SupabaseClient, jobId: string): Promise<Job | null> {
  const { data, error } = await supabase
    .from('job_queue')
    .select(
      'id, job_type, payload, idempotency_key, status, result, retries, max_retries, backoff_seconds, correlation_id, scheduled_at, started_at, completed_at, error_message, created_at',
    )
    .eq('id', jobId)
    .maybeSingle()
